...
group_mapping = {
    ...
    'Fracture': 'Fracture',
    'Heart device': 'Heart device',
    'Hiatal hernia': 'Hiatal hernia',
    'Interstitial pattern': 'Interstitial pattern',
    'Nodule/Mass': 'Nodule/Mass',
    'Osteosynthesis/suture material': 'Osteosynthesis/suture material',
    'Pleural thickening': 'Pleural thickening',
    'Postoperative change': 'Postoperative change',
    'Prosthesis/endoprosthesis': 'Prosthesis/endoprosthesis',
    'Tube': 'Tube',
    # select-only findings (no bounding boxes)
...
from models import db, AccessCode, ActivityLog, Admin
import io
import shortuuid
import pandas as pd
...
# In-memory DB: every server restart starts completely fresh
...
# Fresh DB every restart — create tables and seed defaults
...
# ---------------------------------------------------------------------------
# In-memory report store (resets on server restart)
# ---------------------------------------------------------------------------
# _report_store[access_code] = { case_id: { 'result': <payload>, 'findings': str,
#     'green_score': float, 'time_spent_ms': int, 'timer_checkpoint_ms': int } }
_report_store = {}

def _get_report_count(access_code):
    """Return number of scored reports stored in memory for this access code."""
    return len(_report_store.get(access_code, {}))

def _get_report_entries(access_code):
    """Return list of in-memory report entries for an access code, sorted by insertion order."""
    return list(_report_store.get(access_code, {}).values())

# ---------------------------------------------------------------------------
# In-memory localize store (resets on server restart)
# ---------------------------------------------------------------------------
# _localize_store[access_code] = [ { 'case_id': str, 'selections_json': str,
#     'time_spent_ms': int, 'timer_checkpoint_ms': int, 'correct_count': int,
#     'incorrect_count': int, 'localize_cases_completed_snapshot': int,
#     'timestamp': str } ]
_localize_store: dict[str, list[dict]] = {}

def _get_localize_rows(access_code):
    """Return list of localize case dicts for an access code, in insertion order."""
    return _localize_store.get(access_code, [])

def _get_localize_count(access_code):
    """Return number of localize cases stored in memory for this access code."""
    return len(_localize_store.get(access_code, []))

def _add_localize_row(access_code, row_dict):
    """Append a localize case dict to the in-memory store."""
    _localize_store.setdefault(access_code, []).append(row_dict)
...
def progress_status():
    ...
    loc_cases = _get_localize_count(access.code)
    report_cases = _get_report_count(access.code)
...
def progress_summary():
    ...
    if not access:
        return jsonify({'error': 'not found'}), 404
    rows = _get_localize_rows(access.code)

    cases_total = len(rows)
    correct_conditions = sum(int(r.get('correct_count') or 0) for r in rows)
    incorrect_conditions = sum(int(r.get('incorrect_count') or 0) for r in rows)
    total_time_ms = sum(int(r.get('time_spent_ms') or 0) for r in rows)
    last_timer_checkpoint_ms = 0
    if rows:
        try:
            last_timer_checkpoint_ms = max(int(r.get('timer_checkpoint_ms') or 0) for r in rows)
        except Exception:
            last_timer_checkpoint_ms = 0
...
def report_summary():
    ...
    if not access:
        return jsonify({'error': 'not found'}), 404
    entries = _get_report_entries(access.code)
    report_count = len(entries)
    green_scores = [e['green_score'] for e in entries if e.get('green_score') is not None]
    avg_green = sum(green_scores) / len(green_scores) if green_scores else None
    total_time_ms = sum(int(e.get('time_spent_ms') or 0) for e in entries)
    last_timer_checkpoint_ms = 0
    if entries:
        try:
            last_timer_checkpoint_ms = max(int(e.get('timer_checkpoint_ms') or 0) for e in entries)
        except Exception:
            last_timer_checkpoint_ms = 0
    def _fmt(ms: int) -> str:
        if not ms:
...
def api_localize_guided_next():
    ...
    try:
        if access and current_case:
            ...
            prev_rows = _get_localize_rows(access.code)
            prev_total_time = sum(int(r.get('time_spent_ms') or 0) for r in prev_rows)
...
def get_report_case():
    access = AccessCode.query.filter_by(code=session.get('access_code')).first()
    advance = False
    completed = _get_report_count(access.code) if access else 0
...
def guided_report_log():
    ...
    try:
        current_completed = _get_report_count(access.code)
        advance_after = bool(data.get('advance_after'))
        entries = _get_report_entries(access.code)
        prev_checkpoint = max((int(e.get('timer_checkpoint_ms') or 0) for e in entries), default=0)
        new_timer = int(prev_checkpoint + (time_spent_ms or 0))
        _report_store.setdefault(access.code, {})[case_id] = {
            'green_score': None,
            'time_spent_ms': time_spent_ms,
            'timer_checkpoint_ms': new_timer,
            'findings': '',
            'result': {}
        }
        return jsonify({'ok': True, 'cases_completed': _get_report_count(access.code)})
    except Exception as e:
        return jsonify({'error': 'store', 'detail': str(e)}), 500
...
def debug_ordering():
    ...
    return jsonify({
        ...
        'report_completed': _get_report_count(access.code) if access else 0
    })
...
def submit_report():
    ...
    if not case_data:
        ...
        try:
            ac = session['access_code']
            entries = _get_report_entries(ac)
            prev_checkpoint = max((int(e.get('timer_checkpoint_ms') or 0) for e in entries), default=0)
            new_checkpoint = int(prev_checkpoint + (time_spent_ms or 0))
            _report_store.setdefault(ac, {})[case_id] = {
                'green_score': 1.0, 'time_spent_ms': time_spent_ms,
                'timer_checkpoint_ms': new_checkpoint, 'findings': '',
                'result': {'skipped': True, 'reason': 'case_not_found', 'case_id': case_id}
            }
            next_case_id = _get_ordered_report_case(_get_report_count(ac))
            return jsonify({
                'skipped': True,
    ...
    # ----- In-memory logging and response -----
    ac = session['access_code']
    access_row = AccessCode.query.filter_by(code=ac).first()
    pre_count = _get_report_count(ac)
    # Block further practice submissions if cap reached or post-test already taken
    if access_row and access_row.took_report_pre and pre_count >= REPORT_POST_REQUIRED:
...
def complete_case():
    ...
    prev_rows = _get_localize_rows(session['access_code'])
    prev_total_time = sum(int(r.get('time_spent_ms') or 0) for r in prev_rows)
...
def user_timer_checkpoint():
    ...
    rows = _get_localize_rows(session['access_code'])
    if not rows:
        return jsonify({'error': 'no case found to checkpoint'}), 404
    try:
        rows[-1]['timer_checkpoint_ms'] = value
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    return jsonify({'status': 'ok', 'timer_checkpoint_ms': value})
...
def admin_analytics():
    ...
    for ac in codes:
        enriched.append({
            ...
            'report_cases_completed': _get_report_count(ac.code)
        })
...
def admin_export_code_json():
    ...
    try:
        ...
        base = {
            ...
            'report_cases_completed': _get_report_count(code_value)
        }
        ...
        # User case logs (localize practice, in-memory, resets on restart)
        user_case_logs = _get_localize_rows(code_value)

        # Report logs (in-memory, resets on restart)
        report_logs = _get_report_entries(code_value)
...
def admin_delete_code():
    ...
    try:
        ...
        from models import ActivityLog, BoundingBoxLog
        ...
        # Clear in-memory report and localize stores for this code
        _report_store.pop(code_value, None)
        _localize_store.pop(code_value, None)
...
def admin_delete_all_codes():
    try:
        from models import ActivityLog, BoundingBoxLog, AccessCode
        # Delete in order to satisfy FKs
        # Delete bounding boxes
        BoundingBoxLog.query.delete()
        # Delete activity logs
        ActivityLog.query.delete()
        # Clear all in-memory report data
        _report_store.clear()
        _localize_store.clear()
